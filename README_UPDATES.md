...
### Загрузить базу упражнений с GIF

```bash
npm run exercises:fetch
```

Это займет **~15-30 минут** при первом запуске (загружает ~650 GIF файлов).
При повторных запусках будет быстро (~10 секунд).

**Что произойдет:**
1. Загрузит ~800 упражнений
2. Скачает GIF анимации в `public/exercises/gifs/`
...
1. **Загрузите упражнения:**
   ...
   npm run exercises:fetch
   ```
   ☕ Выпейте кофе пока загружаются GIF (~20 минут)
